...
# -------------------------------------------------------------
# HELPER: Safe Float Converter (Menangani #DIV/0!, #VALUE!, dsb)
# -------------------------------------------------------------
def safe_float(val, default=np.nan):
    try:
        if pd.isna(val):
            return default
        v_str = str(val).strip()
        if v_str in ['#DIV/0!', '#VALUE!', '#N/A', '#REF!', 'nan', 'None', '', '-']:
            return default
        return float(v_str)
    except Exception:
        return default
...
def get_action_recommendation(
    ...
    try:
        c = int(safe_float(cluster, 0))
        u = int(safe_float(flag_utilisasi, 0))
        p = int(safe_float(flag_pendapatan, 0))
        s = int(safe_float(flag_skor_kredit, 0))
...
@st.cache_resource
def load_pipeline():
    if Path("model_gagal_bayar (1).joblib").exists():
        return joblib.load("model_gagal_bayar (1).joblib")
    elif Path("model_gagal_bayar.joblib").exists():
        return joblib.load("model_gagal_bayar.joblib")
    else:
        raise FileNotFoundError("File model joblib tidak ditemukan.")
...
try:
    ...
    st.info(
        "Pastikan file model `.joblib` dan 'custom_transformers.py'"
...
def load_debitur_dataset():
    paths = [
        BASE_DIR / "data" / "hasil_prediksi_cluster_gagal_bayar (2).csv",
        BASE_DIR / "hasil_prediksi_cluster_gagal_bayar (2).csv",
        BASE_DIR / "data" / "hasil_prediksi_cluster_gagal_bayar (1).csv",
        BASE_DIR / "hasil_prediksi_cluster_gagal_bayar (1).csv",
    ]
    for p in paths:
        if p.exists():
            return pd.read_csv(p)
    raise FileNotFoundError("File CSV debitur tidak ditemukan di folder `data/` maupun root.")
...
with tab_detail:
    ...
    try:
        ...
        if selected_id:
            ...
            # ---------------------------------------------------------
            raw_pd_float = safe_float(row.get("y_pred_proba (sigmoid)", None))
            pred_class_val = safe_float(row.get("y_pred (dibulatkan, threshold=0.5)", None))
            pred_class_int = int(pred_class_val) if not np.isnan(pred_class_val) else (1 if raw_pd_float >= 0.5 else 0)
            ...
            if not np.isnan(raw_pd_float):
                ...
                with res_col3:
                    cluster_desc = (
                        "Early Intervention" if int(safe_float(cluster_val, 0)) == 0 else "Intensive"
                    )
            ...
            # ---------------------------------------------------------
            plafon = safe_float(row.get("plafon_pinjaman", 0), 0)
            agunan_awal = safe_float(row.get("nilai_agunan_awal", 0), 0)
            agunan_kini = safe_float(row.get("nilai_agunan_kini", 0), 0)
            tenor = safe_float(row.get("tenor_bulan", 0), 0)
            pendapatan = safe_float(row.get("pendapatan_bulanan", 0), 0)
            ...
            with c1:
                st.metric(
                    ...
                    value=f"Rp {plafon:,.0f}" if plafon > 0 else "Rp 0",
                )
                st.metric(
                    label="Tenor Pinjaman",
                    value=f"{int(tenor)} Bulan" if tenor > 0 else "-",
                )
            ...
            # Format Rasio LTV & DTI dari CSV (menggunakan safe_float agar aman dari string '#DIV/0!')
            ltv_awal_num = safe_float(row.get("ltv_awal", None))
            ltv_kini_num = safe_float(row.get("ltv_kini", None))
            dti_num = safe_float(row.get("dti_ratio", None))

            ltv_awal_str = (
                f"{ltv_awal_num * 100:.2f}%" if not np.isnan(ltv_awal_num) else "-"
            )
            ltv_kini_str = (
                f"{ltv_kini_num * 100:.2f}%" if not np.isnan(ltv_kini_num) else "-"
            )
            dti_str = f"{dti_num * 100:.2f}%" if not np.isnan(dti_num) else "-"
            ...
            data_ringkasan = {
                ...
                "Nilai": [
                    ...
                    action_singkat,
                    f"Rp {plafon:,.0f}" if plafon > 0 else "-",
                    f"Rp {agunan_awal:,.0f}" if agunan_awal > 0 else "-",
                    (
                        f"Rp {agunan_kini:,.0f}"
                        if agunan_kini > 0
                        else "-"
